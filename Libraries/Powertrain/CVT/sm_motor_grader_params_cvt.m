function HMPST = sm_motor_grader_params_cvt(VehicleData)
...
wRadR = VehicleData.tire.param.DIMENSION.UNLOADED_RADIUS;
...
HMPST.DriveshaftF.D = 1e3*10; % N*m/rad

HMPST.DriveshaftR.J  = 1/8; % kg*m^2
HMPST.DriveshaftFL.J = 1/4; % kg*m^2
HMPST.DriveshaftFR.J = 1/4; % kg*m^2
HMPST.DriveshaftRL.J = 1/4; % kg*m^2
HMPST.DriveshaftRR.J = 1/4; % kg*m^2
...
xFinal = [1.4749   27.0000   21.0000   24.0000   24.0000];
...
HMPST.Veh.Mass      = ...
    VehicleData.frontAxle.mass+VehicleData.mainFrame.mass+VehicleData.rearFrame.mass;
HMPST.Veh.Track     = 2.65;%VehicleData.SuspF.Track; % m
HMPST.Veh.Wheelbase = 7; %abs(VehicleData.Body.RA); % m
HMPST.Veh.CG.FA2CGx    = 5;%abs(VehicleData.Body.CG(1)); % m
HMPST.Veh.CG.RA2CGx    = HMPST.Veh.Wheelbase-HMPST.Veh.CG.FA2CGx; % m
HMPST.Veh.CG.FA2CGz    = 1.2563;%abs(VehicleData.Body.CG(3)); % m
...
HMPST.eCVT.Generator.trqSpd.t     = [.09, .08, .07, 0]*1000*2*4*4;  % N*m
HMPST.eCVT.Generator.trqSpd.w     = [0, 3750, 7500, 8000]*2;    % rpm
...
HMPST.eCVT.Motor.trqSpd.t         = [.09, .08, .07, 0]*1000*2*4*4;  % N*m
HMPST.eCVT.Motor.trqSpd.w         = [0, 3750, 7500, 8000]*2;    % rpm
...
HMPST.eCVT.nGearbox               = 3/3;

HMPST.eCVT.Control.Kp             = 10/100*2*2;   % -
HMPST.eCVT.Control.Ki             = 0.5/100*2;  % -
HMPST.eCVT.Control.tFilt          = 0.01;       % sec
...
HMPST.MB.Control.NoHRB.Ki = 0.002*2;
HMPST.MB.Control.NoHRB.Kp = 0.01*2;

HMPST.MB.Control.WithHRB.Ki = 0.002*0.01/2/10 *2;
HMPST.MB.Control.WithHRB.Kp = 0.005*0.01/4 *2;

HMPST.MB.Control.tFilt = 0.1*2 /2*2;

HMPST.MB.Hydrostatic.pressGain = 4;
HMPST.MB.Abstract.pressGain = 3;
...
% Tires, 3D
HMPST.Tire.Contact = simscape.multibody.tirread('KT_MF_Tool_550_80_R30.tir');

% Tires, 1D
HMPST.TireMF1D.FA.fZrated    = 3000*2; % N
HMPST.TireMF1D.FA.fXrated    = 3500*2; % N
...
HMPST.TireMF1D.RA.fZrated    = 3000*2; % N
HMPST.TireMF1D.RA.fXrated    = 3500*2; % N
